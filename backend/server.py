...
url2 = "https://trueway-directions2.p.rapidapi.com/FindDrivingRoute"
...
def geocode():
    if request.method == 'POST':
        ...
        long = float(json.loads(response.text)[
...
def displayDirection():
    ...
    querystring2 = {"stops":stops_arranged}

    response2 = req.request("GET", url2, headers=headers2, params=querystring2)
    ...
    # create map
    m = folium.Map(location=[loc_y.mean(), loc_x.mean()], zoom_start=12)
    # add home
    folium.Marker([loc_y[0], loc_x[0]], popup='Home',
                  icon=folium.Icon(color=' #8F00FF', icon='home')).add_to(m)

    # add markers
    for i in N:
        folium.Marker([loc_y[i], loc_x[i]], popup=(
            loc_y[i], loc_x[i])).add_to(m)

    folium.PolyLine(points, weight=5, opacity=1).add_to(m)


    # add lines
    for i, j in sorted_coords[:-1]:
        folium.PolyLine(locations=[[loc_y[i], loc_x[i]], [
                        loc_y[j], loc_x[j]]], color='red', weight=2.5, opacity=1).add_to(m)
